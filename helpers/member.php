...
if (!function_exists('rasamalaSanitizeMemberSessionContent')) {
    function rasamalaSanitizeMemberSessionContent(&$main_content)
    {
        ...
        // Enhance Title Basket UI/UX
        rasamalaEnhanceTitleBasketPage($main_content);
    }
}

if (!function_exists('rasamalaEnhanceTitleBasketPage')) {
    function rasamalaEnhanceTitleBasketPage(&$main_content)
    {
        if (!isset($main_content)) return;
        if (!isset($_GET['sec']) || $_GET['sec'] !== 'title_basket') return;

        // Extract count of titles in basket if present
        $count = 0;
        if (preg_match('/(\d+)\s+title\(s\)\s+on\s+basket/i', $main_content, $matches)) {
            $count = (int)$matches[1];
        } elseif (isset($_SESSION['m_mark_biblio']) && is_array($_SESSION['m_mark_biblio'])) {
            $count = count($_SESSION['m_mark_biblio']);
        }

        // Clean legacy heading text
        $main_content = str_replace('My Title Basket', '', $main_content);
        $main_content = str_replace('Keranjang Judul Saya', '', $main_content);

        if ($count < 1) {
            $empty_html = '
            <div class="rasamala-basket-header mb-4 p-3 p-md-4 rounded-3 border bg-surface shadow-sm">
                <div class="d-flex align-items-center justify-content-between">
                    <div>
                        <h4 class="fw-bold mb-1 d-flex align-items-center gap-2 text-theme">
                            <i class="fas fa-shopping-basket text-primary" aria-hidden="true"></i>
                            <span>' . __('My Title Basket') . '</span>
                        </h4>
                        <p class="text-muted small mb-0">' . __('Kelola koleksi buku simpanan Anda untuk reservasi peminjaman.') . '</p>
                    </div>
                </div>
            </div>
            <div class="rasamala-basket-empty-card p-5 text-center my-4 bg-surface border rounded-3 shadow-sm">
                <div class="basket-empty-icon-wrap mb-3 text-muted opacity-50">
                    <i class="fas fa-shopping-basket fa-4x" aria-hidden="true"></i>
                </div>
                <h5 class="fw-bold mb-2 text-theme">' . __('Keranjang Judul Anda Kosong') . '</h5>
                <p class="text-muted mb-4 mx-auto" style="max-width: 480px;">' . __('Anda belum menambahkan judul buku ke dalam keranjang. Cari dan simpan koleksi favorit Anda saat menjelajahi OPAC.') . '</p>
                <a href="index.php" class="btn btn-primary rounded-pill px-4 py-2 shadow-sm fw-bold">
                    <i class="fas fa-search me-2" aria-hidden="true"></i> ' . __('Jelajahi Katalog Buku') . '
                </a>
            </div>';

            $main_content = preg_replace('/<div class="memberBasketInfo">.*?<\/div>\s*(?:<table\b.*?<\/table>)?/is', $empty_html, $main_content);
            return;
        }

        // Build modern header & action bar
        $header_html = '
        <div class="rasamala-basket-header mb-4 p-3 p-md-4 rounded-3 border bg-surface shadow-sm">
            <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
                <div>
                    ...
                    <p class="text-muted small mb-0">' . __('Pilih judul buku yang ingin Anda pesan / reservasi atau hapus dari daftar keranjang.') . '</p>
                </div>
                <div class="d-flex align-items-center gap-2">
                    <span class="badge bg-primary-subtle text-primary border border-primary-subtle rounded-pill px-3 py-2 fs-6 fw-bold">
                        <i class="fas fa-book me-1.5" aria-hidden="true"></i> ' . sprintf(__('%d Judul'), $count) . '
                    </span>
                </div>
            </div>
        </div>';
        ...
        $action_bar = '
        <div class="rasamala-basket-actions-toolbar d-flex flex-wrap align-items-center justify-content-between gap-2 my-3 p-3 bg-surface rounded-3 border shadow-sm">
            <div class="d-flex flex-wrap align-items-center gap-2">
                <a href="index.php?p=member&sec=title_basket" class="btn btn-emerald shadow-sm basket reserve fw-bold px-3">
                    <i class="fas fa-bookmark me-1.5" aria-hidden="true"></i> ' . __('Reservasi Judul Terpilih') . '
                </a>
                <a href="index.php?p=member&sec=title_basket" class="btn btn-outline-danger shadow-sm basket clearOne px-3">
                    <i class="fas fa-trash-alt me-1.5" aria-hidden="true"></i> ' . __('Hapus yang Dipilih') . '
                </a>
            </div>
            <div>
                <a href="index.php?p=member&sec=title_basket" class="btn btn-outline-secondary btn-sm basket clearAll px-3" postdata="clear_biblio=1">
                    <i class="fas fa-eraser me-1.5" aria-hidden="true"></i> ' . __('Kosongkan Keranjang') . '
                </a>
            </div>
        </div>';

        // Replace legacy memberBasketInfo block
        $main_content = preg_replace('/<div class="memberBasketInfo">.*?<\/div>/is', $header_html . "\n" . $action_bar, $main_content);

        // Improve table styling & headers
        $main_content = str_replace(
            'class="memberBasketList table table-striped"',
            'class="memberBasketList table table-hover align-middle rasamala-basket-table shadow-sm border rounded-3 overflow-hidden mb-4"',
            $main_content
        );

        $main_content = str_replace(
            '<th width="5%">Remove</th>',
            '<th width="5%" class="text-center bg-surface border-bottom py-3"><input type="checkbox" id="rasamala-basket-select-all" class="form-check-input" title="' . themeEscape(__('Pilih Semua')) . '" aria-label="' . themeEscape(__('Pilih Semua')) . '"></th>',
            $main_content
        );

        $main_content = str_replace(
            '<th>Title</th>',
            '<th class="bg-surface border-bottom text-uppercase tracking-wider text-muted small fw-bold py-3">' . __('Judul Buku / Koleksi') . '</th>',
            $main_content
        );

        // Improve item checkbox styling
        $main_content = str_replace(
            'class="basketItem"',
            'class="basketItem form-check-input"',
            $main_content
        );

        // Inject select-all script
        $select_all_script = '
        <script>
        document.addEventListener("DOMContentLoaded", function() {
            var selectAll = document.getElementById("rasamala-basket-select-all");
            if (selectAll) {
                selectAll.addEventListener("change", function() {
                    var items = document.querySelectorAll(".basketItem");
        ...
        $main_content .= $select_all_script;
    }
}
